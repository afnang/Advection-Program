...
/* function to calculate horizontal velocity*/
  float horizontalVelocity(float z, float z0, float uStar, float vk){
    if (z > z0 && z != 0 && z0 != 0 && uStar != 0 && vk != 0) {
        return (uStar/vk) * log(z/z0);
    }
    /* Initial value of dt when horizontal velocity is 1 */
    else if (z == -1) {
        return 1.0;
    } else {
        return 0;
...
int main(){
  ...
  // Horizontal velocity not a constant in part 3 therefore this part can be removed 
  ...
  /* New parameters required by task 2 part 3*/
  const float uStar = 0.2; // Friction velocity (m/s)
  const float z0 = 1.0;     // Roughness length (m)
  const float vk = 0.41;     // Von Kármán constant
  float averageU[NX];
  ...
  float dt = CFL / ((fabs(horizontalVelocity(-1, z0, uStar, vk)) / dx) + (fabs(vely) / dy));
  ...
  printf("End time            = %g\n", dt*(float) nsteps);
  printf("Distance advected x = %g\n", horizontalVelocity(-1, z0, uStar, vk)*dt*(float) nsteps);
  printf("Distance advected y = %g\n", vely*dt*(float) nsteps);
  
  /*** Place x points in the middle of the cell ***/
  /* LOOP 1 can be parallelised*/
  ...
  /* LOOP 4 cannot be parallelised*/
  /* Data dependencies would make it unsafe as you cannot guarantee 
    the order that the data is being written to the file*/
  for (int i=0; i<NX+2; i++){
    for (int j=0; j<NY+2; j++){
  ...
  /* LOOP 5 cannot be parallelised as if it was parallelised,
   the time steps would carry out in the correct order */
  for (int m=0; m<nsteps; m++){
    
    /*** Apply boundary conditions at u[0][:] and u[NX+1][:] ***/
    /* LOOP 6 can be parallelised*/
    ...
    /* LOOP 8 can be parallelised*/
    #pragma omp parallel for default(none) shared(dx, dy, u, dudt, y)
    for (int i=1; i<NX+1; i++){
      for (int j=1; j<NY+1; j++){
	dudt[i][j] = -horizontalVelocity(y[j], z0, uStar, vk) * (u[i][j] - u[i-1][j]) / dx - vely * (u[i][j] - u[i][j-1]) / dy;
      }
    }

    dt = CFL / ( (fabs(horizontalVelocity(y[NX+1], z0, uStar, vk)) / dx) + (fabs(vely) / dy) );
    
    /*** Update u from t to t+dt ***/
    /* Loop over points in the domain but not boundary values */
    /* LOOP 9 can be parallelised*/
  ...
  /* LOOP 10 cannot be parallelised as accuracy of the order of data being written
   cannot be guaranteed which would impact how the final graph would look*/
  for (int i=0; i<NX+2; i++){
    for (int j=0; j<NY+2; j++){
  ...
  fclose(finalfile);
  /* Task 2 part 4*/
  /* Calculate averaged values of u and add to array */ 
  for (int i=1; i<NX+1; i++) {
    float meanU = 0;
    float totalU = 0;
    for (int j=1; j<NY+1; j++) {
